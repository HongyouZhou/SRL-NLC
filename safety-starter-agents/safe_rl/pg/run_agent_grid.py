...
from scripts.ConstraintMaskModule import *
...
def run_polopt_agent_grid(
    ...
    env = env_fn()
    env.set_max_step(steps_per_epoch)
    ...
    # load constraint interpreter
    # d_model = 16
    # constraint_interpreter = ConstraintMaskModule(obs_size=7,
    #                                               obs_channels=3,
    #                                               description_size=10,
    #                                               conv_filter=10,
    #                                               kernel_size=3,
    ...
    # )
    #
    # output = constraint_interpreter((test_constraint, test_obs))
    # checkpoint_path = "/home/hongyou/TUB/SRL-NLC/safety-starter-agents/scripts/dummy/cp-0001.ckpt"
    # with tf.device('/CPU:0'):
    #     constraint_interpreter.load_weights(checkpoint_path)
    #     constraint_interpreter.summary()
    ...
    approx_ent = tf.reduce_mean(input_tensor=-logp)
    ...
    # Obs/act shapes
    obs_shape = (env.observation_space['image'].shape[:-1]) + (env.observation_space['image'].shape[-1] + 2,)
    ...
    if agent.use_penalty:
        with tf.compat.v1.variable_scope("penalty"):
            ...
            penalty_param = tf.compat.v1.get_variable(
                "penalty_param",
    ...
    if agent.clipped_adv:
        min_adv = tf.compat.v1.where(
            adv_ph > 0, (1 + agent.clip_ratio) * adv_ph, (1 - agent.clip_ratio) * adv_ph
        )
        surr_adv = tf.reduce_mean(input_tensor=tf.minimum(ratio * adv_ph, min_adv))
    else:
        surr_adv = tf.reduce_mean(input_tensor=ratio * adv_ph)

    # Surrogate cost
    surr_cost = tf.reduce_mean(input_tensor=ratio * cadv_ph)
    ...
    vc_loss = tf.reduce_mean(input_tensor=(cret_ph - vc) ** 2)
    ...
    avoid_obj = env.avoid_obj
    cost_lim = env.hc
    training_package.update(dict(cost_lim=env.hc))
    cur_penalty = 0
    cum_cost = 0
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            ...
            input_x = np.append(o['image'], mask, axis=2)

            # Get outputs from policy
            get_action_outs = sess.run(get_action_ops, feed_dict={x_ph: input_x[np.newaxis]})
            ...
            # Step in environment
            o2, r, d, info = env.step(int(a[0]))

            # o2_img_pred = np.expand_dims(np.float32(o2['image']), axis=0)
            # cc = np.zeros((10), dtype=int)
            # code = wrapper.encode_mission(o['mission'])
            # cc[0:len(code)] = code
            # code = np.expand_dims(cc, axis=0)
            # constraint_mask = constraint_interpreter((code, o2_img_pred))
            ...
            if agent.reward_penalized:
                ...
                buf.store(input_x, a, r_total, v_t, 0, 0, logp_t, pi_info_t)
            else:
                buf.store(input_x, a, r, v_t, c, vc_t, logp_t, pi_info_t)
            ...
            if terminal or (t == local_steps_per_epoch - 1):
                ...
                if d and not (ep_len == max_ep_len):
                    ...
                    mask = np.append(constraint_mask, cost_budget_mask, axis=2)
                    input_x = np.append(o["image"], mask, axis=2)
                    feed_dict = {x_ph: input_x[np.newaxis]}
                    if agent.reward_penalized:
        ...
        logger.log_tabular("EpLen")
        logger.log_tabular("CumulativeCost", cumulative_cost)
        logger.log_tabular("CostRate", cost_rate)

        # Value function values
        logger.log_tabular("VVals")
        logger.log_tabular("CostVVals")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", type=str, default="MiniGrid-HazardWorld-R-v0")
